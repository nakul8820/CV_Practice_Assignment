# CV_Practice_Assignment
I will be using this repo to implement and solve coding assignments from different universities as a way to sharpen my skills and translate my academic learning into practical code."

Following Assignment is from IIT-M . 
...
number of filters in each layer : 323232, 646464, ...
...
(UNGRADED, OPTIONAL) Visualise all the filters in the first layer of your best model for a random image from the test set. If there are 64 filters in the first layer plot them in an 8×88 \times 88×8 grid.
(UNGRADED, OPTIONAL) Apply guided back-propagation on any 101010 neurons in the CONV5 layer and plot the images which excite this neuron. The idea again is to discover interesting patterns which excite some neurons. You will draw a 10×110 \times 110×1 grid below with one image for each of the 101010 neurons.
Question 5 (10 Marks)
...
ImageNet has 100010001000 classes and hence the last layer of the pre-trained model would have 100010001000 nodes. However, the naturalist dataset has only 101010 classes. How will you address this?
